...
public class AdminPojo {
  ...
  try { if (con != null) con.close(); con=null; } catch (SQLException ex2) { }
    ...
    public List<AdminModel> RetailerShow() {
        ...
        try {
            con = Database.getConnection();
            String query = "select SI_NO, R_NAME, PRO_NAME, THANA, ZILLA, AREA, MAN_NAME, M_NUMBER ,D_O_B, D_O_M from retailer_info order by R_NAME";
            ps = con.prepareStatement(query);
            rs = ps.executeQuery();
            while (rs.next()) {
             AdminModel am=new AdminModel();
             ...
             am.setProname(rs.getString(3));
             am.setThana(rs.getString(4));
             am.setZilla(rs.getString(5));
             am.setArea(rs.getString(6));
             am.setManager(rs.getString(7));
             am.setMnumber(rs.getString(8));
             am.setDob(rs.getString(9));
             am.setDom(rs.getString(10));
