        ...
        %>
            <%
                ...
                try {
                    ...
                    String query = "select ORDER_NAME, ORDER_DATE, PRODUCT_NAME, QTY, PURSE_RATE, RATE, DATE from order_delevery where QTY>0 and YEAR(DATE) = '"+ year +"' AND MONTH(DATE) = '"+ month +"'";
                    ps = con.prepareStatement(query);
                    rs = ps.executeQuery();
                    String query1 = "select sum(QTY), sum(QTY*RATE), sum(QTY*PURSE_RATE) from order_delevery where YEAR(DATE) = '"+ year +"' AND MONTH(DATE) = '"+ month +"'";
                    ...
                    String query2="select sum(AMOUNT) from fac_commission where MONTH=? and YEAR=?";
                    ...
                    String query3 = "select MONTHNAME(DATE) from order_delevery where YEAR(DATE) = '"+ year +"' AND MONTH(DATE) = '"+ month +"'";
                    ps = con.prepareStatement(query3);
                    rs3=ps.executeQuery();
                    rs3.next();
                    request.setAttribute("mnth", rs3.getString(1));
                    String query4="select FACTORY_NAME, COM_NAME, AMOUNT, START_DATE, END_DATE from fac_commission where MONTH=? and YEAR=?";
                    ps = con.prepareStatement(query4);
                    ps.setInt(1, month);
                    ps.setInt(2, year);
                    rs4 = ps.executeQuery();
                    String query5="select COST_NAME, AMOUNT, DATE from cost where YEAR(DATE) = '"+ year +"' AND MONTH(DATE) = '"+ month +"'";
                    ps = con.prepareStatement(query5);
                    rs5 = ps.executeQuery();
                    String query6="select sum(AMOUNT) from cost where YEAR(DATE) = '"+ year +"' AND MONTH(DATE) = '"+ month +"'";
                    ps = con.prepareStatement(query6);
                    rs6 = ps.executeQuery();
                    rs6.next();
                    request.setAttribute("tcost", rs6.getLong(1));
                    String query7="select EMP_NAME, COST_NAME, AMOUNT, DATE from emp_cost where YEAR(DATE) = '"+ year +"' AND MONTH(DATE) = '"+ month +"'";
                    ps = con.prepareStatement(query7);
                    rs7 = ps.executeQuery();
                    String query8="select sum(AMOUNT) from emp_cost where YEAR(DATE) = '"+ year +"' AND MONTH(DATE) = '"+ month +"'";
                    ps = con.prepareStatement(query8);
                    rs8 = ps.executeQuery();
                    rs8.next();
                    request.setAttribute("tempcost", rs8.getLong(1));
                    String query9="select sum(AMOUNT) from rtler_commission where YEAR(DATE) = '"+ year +"' AND MONTH(DATE) = '"+ month +"'";
                          ...
                          <table border="2" width="100%" class="table-striped table-responsive">
                            ...
                            <tbody>
                               <%
                                     ...
                                     try {
                                     con = Database.getConnection();
                                         String query2 = "select RETAILER_NAME, NOTE, AMOUNT, DATE from rtler_commission where YEAR(DATE) = '"+ year +"' AND MONTH(DATE) = '"+ month +"'";
                                     ps = con.prepareStatement(query2);
                                     rs = ps.executeQuery();
                                     while (rs.next()) {
                               %>
